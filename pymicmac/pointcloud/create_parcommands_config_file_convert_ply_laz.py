...
def run(inputFolder, outputFile, outputFormat, outputFolder, num):
    ...
    for chunk in chunks(inputFiles, num):
        ...
        for inputFile in chunk:
            ...
            chunkOutputFiles.append(
                'outputData/' + taskName + '/' + convertedFileName)
        ...
        childOutputCommand = etree.SubElement(childOutput, 'command')
        childOutputCommand.text = ' coeman-par-local -d . -c ' + \
            chunkXMLFileName + ' -e outputData -n ' + str(num)
...
def argument_parser():
    ...
    parser.add_argument(
        ...
        '--input',
        default='',
        help='Input folder with ply files',
        type=str,
        required=True)
    parser.add_argument(
        '-o',
    ...
    parser.add_argument(
        ...
        '--num',
        default='8',
        help='Parallelization at level 2 (default is 8)',
        type=int,
        required=True)
    return parser
